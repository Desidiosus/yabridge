...
class Editor {
   public:
    enum class CoordinatePolicy {
        generic,
        prefer_parent_root_offset,
    };
    ...
    Editor(
        ...
        CoordinatePolicy coordinate_policy = CoordinatePolicy::generic);
   ...
   private:
    ...
    /**
     * Get the root-relative offset of the parent window to fix mouse
     * coordinates.
     */
    std::optional<std::array<int16_t, 2>> get_parent_window_root_offset()
        const noexcept;

    /**
     * Compute the legacy translated X11 coordinates used for forwarded
     * ConfigureNotify events.
     */
    std::array<int16_t, 2> get_generic_translated_coordinates() const noexcept;

    /**
     * Compute the translated X11 coordinates used for forwarded
     * ConfigureNotify events.
     */
    std::array<int16_t, 2> get_translated_coordinates(bool is_synthetic_event) const;
     ...
     */
    const CoordinatePolicy coordinate_policy_;
